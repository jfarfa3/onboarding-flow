...
def add_access(access_data: AccessCreate, db=Depends(get_db)):
    try:
...
def get_accesses(db=Depends(get_db)):
    try:
...
def get_access(access_id: str, db=Depends(get_db)):
    try:
...
def update_access(access_id: str, access_data: AccessUpdate, db=Depends(get_db)):
    try:
...
def delete_access(access_id: str, db=Depends(get_db)):
    try:
