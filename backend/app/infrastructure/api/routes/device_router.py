...
from app.application.use_cases.device_use_case import (
    create_device_use_case, get_all_devices_use_case,
    get_device_by_id_use_case, update_device_use_case, delete_device_use_case
)
...
def add_device(device_data: DeviceCreate, db=Depends(get_db)):
    try:
...
def update_device(device_id: str, device_data: DeviceUpdate, db=Depends(get_db)):
    try:
